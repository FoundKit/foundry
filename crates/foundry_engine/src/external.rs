...
/// Manifest file (`subsystem.json`) specification for dynamic standalone subsystems
...
/// External standalone subsystem module discovered dynamically from the filesystem
#[derive(Debug, Clone)]
...
pub fn load_external_subsystems(extra_search_dirs: &[PathBuf]) -> Vec<Box<dyn SubsystemModule>> {
    let mut modules: Vec<Box<dyn SubsystemModule>> = Vec::new();
    let mut seen_slugs = std::collections::HashSet::new();

    let mut search_paths = Vec::new();
    search_paths.extend(extra_search_dirs.iter().cloned());
    search_paths.extend(get_external_search_paths());
