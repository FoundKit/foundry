---
title: "02. 存储层实现与动态模型 (foundry_storage)"
description: "阶段 2：深度剖析数据库连接池、Redis 会话缓存、动态模型引擎 RecordStore 与 CRUD 实现。"
---

> **源码导读导航**：⬅️ 上一篇：[01. 核心类型与接口定义 (foundry_core)](./01-core/) · [📋 学习指南与总览](./) · ➡️ 下一篇：[03. 认证与权限控制系统 (foundry_auth)](./03-auth/)
...
### 📚 阶段导航
- ⬅️ **上一阶段**：[01. 核心类型与接口定义 (foundry_core)](./01-core/)
- 📋 **学习指南总览**：[返回源码深度理解总览](./)
- ➡️ **下一阶段**：[03. 认证与权限控制系统 (foundry_auth)](./03-auth/)
